...
def _write_active_workflow(target_root: Path, slug: str, *, self_host: bool = False) -> Path:
    active_path = target_root / ".gsdlc" / "release" / "active-workflow.json"
    active_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "workflow": "genesis_sdlc.standard",
        "version": VERSION,
        "install_mode": "self_host" if self_host else "default",
        "slug": slug,
        ...
        "territory_boundary": {
            ...
            "authoring_allowed_in_self_host_install": [
                "specification/standards/",
            ] if self_host else [],
            "release_managed": [
                ".genesis/",
...
def _load_active_workflow_payload(target_root: Path) -> dict[str, object]:
    active_path = target_root / ".gsdlc" / "release" / "active-workflow.json"
    if not active_path.exists():
        return {}
    try:
...
def install(
    ...
    *,
    ...
    if audit_only:
        active_workflow_payload = _load_active_workflow_payload(target_root)
        ...
        territory_boundary = active_workflow_payload.get("territory_boundary", {})
        allowed_in_self_host = set()
        if isinstance(territory_boundary, dict):
            allowed = territory_boundary.get("authoring_allowed_in_self_host_install", [])
            if isinstance(allowed, list):
                allowed_in_self_host = {str(entry).rstrip("/") for entry in allowed if isinstance(entry, str)}
        install_mode = active_workflow_payload.get("install_mode", "default")
        forbidden = [
            target_root / "specification" / "standards",
        ]
        present_forbidden = []
        for path in forbidden:
            relative = str(path.relative_to(target_root)).rstrip("/")
            if path.exists() and not (install_mode == "self_host" and relative in allowed_in_self_host):
                present_forbidden.append(relative)
        return {
    ...
    active_workflow = _write_active_workflow(target_root, slug, self_host=self_host)
