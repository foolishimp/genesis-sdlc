...
**Acceptance Criteria**:
- AC-1: A runtime reset operation clears execution-state surfaces under `.ai-workspace/`, including `.ai-workspace/runtime/`
- AC-2: Runtime reset preserves `.genesis/`, `.gsdlc/release/`, `.claude/commands/`, and project-local `specification/`
- AC-3: Cleared runtime-control artifacts such as resolved runtime state, doctor snapshots, backend availability state, and session overrides are regenerated by the normal control-plane compile rather than preserved as install state
- AC-4: Runtime reset returns structured JSON describing what was cleared and what was preserved
- AC-5: Reinstall remains the managed reset path for system-owned install surfaces
